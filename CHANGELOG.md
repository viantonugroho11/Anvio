...
- **The unified gateway had none of the API's hardening** ([#44](https://github.com/viantonugroho11/Anvio/pull/44)). `anvio gateway start` — the bundled daemon CLAUDE.md recommends — serves `/api/*` from a second, hand-rolled implementation, and every decision made in ADR-0018 and ADR-0021 applied only to `apps/api`. It bound `0.0.0.0` with no `assertSafeBinding`; its `resolveAuth` ended `return ctx ?? auth.getDefaultContext()`, the exact fallback ADR-0018 removed from `sessions.controller.ts`; and all three channel webhooks dispatched unverified. The shared pieces now live in `packages/platform` and both surfaces call the same functions rather than equivalent ones. Closes [#42](https://github.com/viantonugroho11/Anvio/issues/42). **Breaking:** `anvio gateway start` now binds `127.0.0.1` and refuses to start on a reachable address with auth disabled — set `ANVIO_GATEWAY_HOST` plus either real auth or `ANVIO_API_ALLOW_INSECURE=true`.
- **`WHATSAPP_VERIFY_TOKEN` fell back to a published default** ([#41](https://github.com/viantonugroho11/Anvio/pull/41)). `'anvio-verify'` sat as the fallback at two sites, so any deployment that never set the variable would complete Meta's webhook handshake for anyone who had read this repository — while looking configured, which is what makes a default secret worse than none. Removed, and `verifyWebhook` now fails closed on an empty token: the original `===` would have matched an empty presented token against an empty configured one, swapping a known-secret hole for a no-secret one. The comparison is also constant-time. Separately, `anvio channels status --json` printed the token's value in its health details; it now reports only whether one is configured. Closes [#39](https://github.com/viantonugroho11/Anvio/issues/39). **Breaking:** a deployment relying on the default now fails its handshake — a deployment whose handshake anyone could already complete.
- **Teams and Matrix webhooks accepted anything from anyone** ([#40](https://github.com/viantonugroho11/Anvio/pull/40)). ADR-0018 marked the three channel webhooks `@Public()` because Meta, Microsoft, and a Matrix homeserver cannot present a user token — but only WhatsApp verified anything, and only on the one-time GET handshake. A post to the Matrix webhook injected a message attributed to any `senderId` the caller named, into an agent that read it and spent model credits acting on it. Each provider is now verified by its own mechanism: HMAC-SHA256 over the raw body for Meta, the application-service `as_token` for Matrix, and a full Bot Framework JWT check for Teams — RS256 against Microsoft's published keys with the algorithm pinned and the audience checked, so neither an `alg: none` forgery nor a validly signed token minted for a different bot is accepted. Implemented on `node:crypto` with no new dependency. Closes [#31](https://github.com/viantonugroho11/Anvio/issues/31). **Breaking:** a Teams or Matrix webhook that worked before now returns 401 until `TEAMS_APP_ID` or `MATRIX_AS_TOKEN` is set, or the deployment sets `ANVIO_API_ALLOW_INSECURE=true`.
